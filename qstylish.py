...
DARK = None
LIGHT = None
with open(os.path.join(dirname, "resources/compiled/dark.qss")) as f:
    DARK = f.read()
with open(os.path.join(dirname, "resources/compiled/light.qss")) as f:
    LIGHT = f.read()


def dark(hot_reload=False):
    from resources.compiled import qstylish_rc
    # if hot_reload:
    #     compile()
    return DARK
